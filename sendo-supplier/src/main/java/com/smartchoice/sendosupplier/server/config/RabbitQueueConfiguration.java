package com.smartchoice.sendosupplier.server.config;
...
import com.smartchoice.sendosupplier.server.service.consumers.CategoryConsumer;
import com.smartchoice.sendosupplier.server.service.consumers.ProductConsumer;
...
public class RabbitQueueConfiguration {
	...
	SimpleMessageListenerContainer categoryRequestContainer(ConnectionFactory connectionFactory,
		...
		container.setQueueNames(QueueName.SENDO_RABBITMQ_QUEUE_NAME_CATEGORY_REQUEST_MAIN);
		container.setMessageListener(categoryRequestListenerAdapter);
		log.info("Listening on {}", QueueName.SENDO_RABBITMQ_QUEUE_NAME_CATEGORY_REQUEST_MAIN);
		return container;
	}
	...
	SimpleMessageListenerContainer productRequestContainer(ConnectionFactory connectionFactory,
		...
		SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
		container.setConnectionFactory(connectionFactory);
		container.setQueueNames(QueueName.SENDO_RABBITMQ_QUEUE_NAME_PRODUCT_REQUEST_MAIN);
		container.setMessageListener(productRequestListenerAdapter);
		log.info("Listening on {}", QueueName.SENDO_RABBITMQ_QUEUE_NAME_PRODUCT_REQUEST_MAIN);
		return container;
	}
}
